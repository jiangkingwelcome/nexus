...
- [ ] **Alist 客户端:** 封装 Axios，实现 `login`, `list`, `get`, `search` 接口，拦截器统一注入 Token
...
- [ ] **下拉刷新:** 强制调用 Alist 接口刷新当前目录缓存

### 2.2 全局搜索 (Search)

- [ ] **索引构建:** App 启动时后台静默拉取 Alist 完整目录树，存入 IndexedDB
...
- [ ] **沙箱隔离:** 使用全屏 `<iframe>` 加载 Alist 返回的直链
...
- [ ] **API 配置页:** 允许用户手动输入 Alist 地址、PocketBase 地址和 Token (用于首次初始化)
...
│         │                   │                   │                  │       │
│         ▼                   ▼                   ▼                  ▼       │
│  • Alist 连接        • HTML 容器         • PocketBase      • PWA 图标      │
